...
export const useAssetsStore = defineStore({
  ...
  actions: {
    ...
    async loadHistory() {
      axios.get('http://127.0.0.1:8000/api/history/?format=json').then((response) => this.history = response.data)
    },
    async loadHistoryChart() {
      return axios.get('http://127.0.0.1:8000/api/history/?format=json')
    }
  },
  getters: {
